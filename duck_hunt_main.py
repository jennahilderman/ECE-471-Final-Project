...
def main(args):
    ...
    while True:
        ...
        if args.move_type == 'manual':
            # manual mode
            result = [
                {"coordinate": pygame.mouse.get_pos(), 'move_type': "absolute"}]
        else:
            if future is None:
                result = noop()
                future = executor.submit(
                    GetLocation, args.move_type, env, current_frame)
            elif future.done():
        ...
        for res in result[:10]:
            ...
            current_frame, level_done, game_done, info = env.step(
                coordinate, move_type)
